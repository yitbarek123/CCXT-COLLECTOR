...
host_data_dir = "/data/ccxt-data/"  # This is the directory on the host

# Iterate over all exchanges in ccxt
for exchange_name in ccxt.exchanges:
    # Name the container based on the exchange
    container_name = f"{exchange_name}-ccxt-trade"
    ...
    try:
        container = client.containers.run(
            image="collector-ccxt",  # Replace with your image name for fetching trades
            name=container_name,
            environment=env_vars,
            volumes={exchange_data_dir: {'bind': exchange_data_dir, 'mode': 'rw'}},  # Mount host dir to /app/data in container
            detach=True
        )
